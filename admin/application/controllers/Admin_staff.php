...
class Admin_staff extends CI_Controller {
            ...
            if ($this->session->userdata('route_path') == 'users/doctor') {
                ...
                if ($this->input->post('add_ads')) {
                    ...
                    if ($this->form_validation->run() === TRUE) {
                        $data['status'] = $this->admin_staff_model->add_admin_staff();
                        if ($data['status']) {
    ...
    function del_staff($id = null) {
        if ($this->session->userdata('route_path') == 'users/doctor') {
            $data['status'] = $this->admin_staff_model->del_staff($id);
            if ($data['status']) {
        ...
        if ($this->session->userdata('user_name')) {
            if ($this->session->userdata('route_path') == 'users/doctor') {
                ...
                if ($this->input->post('update_asd')) {
                    ...
                    if ($this->form_validation->run() === TRUE) {
                        $data['status'] = $this->admin_staff_model->update_admin_staff($this->input->post('a_id'));
                        if ($data['status']) {
