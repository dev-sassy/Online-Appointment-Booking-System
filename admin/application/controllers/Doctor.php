...
class Doctor extends CI_Controller {
            ...
            if ($this->session->userdata('route_path') == 'users/doctor') {
                ...
                if ($this->input->post('add_dr')) {
                    ...
                    if ($this->form_validation->run() === TRUE) {
                        ...
                        $data['status'] = $this->doctor_model->add_doctor($dr_name);
                        if ($data['status']) {
    ...
    function del_doctor($id = null) {
        if ($this->session->userdata('route_path') == 'users/doctor') {
            $data['status'] = $this->doctor_model->del_doctor($id);
            if ($data['status']) {
                $this->session->set_flashdata('successMsg', 'Record Inserted Successfully');
                redirect(base_url() . 'doctor');
            }
            ...
            if ($this->session->userdata('route_path') == 'users/doctor') {
                ...
                if ($this->input->post('update_dr')) {
                    ...
                    if ($this->form_validation->run() === TRUE) {
                        ...
                        $data['status'] = $this->doctor_model->update_doctor($this->input->post('dr_id'), $dr_name);
                        if($data['status']){
